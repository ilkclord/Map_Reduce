 ...
 *
 *    Description: project of linux2021
 *
 *        Version:  1.0
 *        Created:  廿廿一年四月廿八日 一時53分44秒
 *       Revision:  none
 *       Compiler:  gcc
 *
 *         Author:  ilkclord
...
typedef int hashfunc_t(void *key, hash_t *hash, uint32_t *bkt , int count);
typedef int cmp_t(struct ht_node *n, void *key , int size);
 ...
 */
static inline struct ht_node *ht_find(struct htable *h, void *key , int count)
{
    hash_t hval;
    uint32_t bkt;
    h->hashfunc(key, &hval, &bkt ,count);

    struct list_entry *head = &h->buckets[bkt], *iter;
    list_for_each (head, iter) {
        struct ht_node *n = list_element(iter, struct ht_node, list);
        if (n->hash == hval) {
            int res = h->cmp(n, key , count);
            if (!res) return n;
 ...
 * Return 0 if success.
 */
static inline int ht_insert(struct htable *h, struct ht_node *n, void *key , int count)
{
    hash_t hval;
    uint32_t bkt;
    h->hashfunc(key, &hval, &bkt , count);
    ...
    list_for_each (head, iter) {
        ...
        if (tmp->hash >= hval) {
            int cmp = h->cmp(tmp, key , count);
            if (!cmp) /* already exist */
                return -1;
            if (cmp > 0) {
                list_add_prev(iter, &n->list);//MMM
                return 0;
            }
        }
    }
...
static inline struct ht_node *ht_get_first(struct htable *h, uint32_t bucket)
{
    ...
    return list_first(head, struct ht_node, list); // NNN
}
...
static int wc_cmp(struct ht_node *n, void *key , int size)
{
    return __wc_cmp(n, key, 0 , size);
}

static int wc_cmp_main(struct ht_node *n, void *key , int size)
{
    return __wc_cmp(n, key, 1 , size);
}
...
{
    code_min = get_code("a" , 0), code_max = get_code("zzzzzzzzzz" , 0);
...
static int hash_bucket(void *key, hash_t *hash, uint32_t *bkt , int count)
{
    uint32_t code = get_code((char *) key , count);
...
int wc_add_word(uint32_t tid, char *word, uint32_t count)
{
    ...
    struct ht_node *n;
    if (!(n = ht_find(&cache->htable, word , count))) {
        ...
        ht_insert(&cache->htable, &w->node, word , count);
    } else
...
static int __merge_results(uint32_t tid, uint32_t j, struct wc_cache *wcc)
{
    ...
    for (; iter; iter = ht_get_next(&wcc->htable, j, iter)) {
        ...
        char *wd = GET_WORD(iw);
        struct ht_node *n;
        if ((n = ht_find(&cache->htable, wd , 0))) {
            ...
            ht_insert(&cache->htable,  &iw->node_main, wd , 0);//PPP
    }
    return 0;
}
...
int wc_merge_results(uint32_t tid, uint32_t n_threads)
{
    ...
    if ((tid == (n_workers - 1))) wk_bend += n_buckets % n_workers ;//QQQ
    ...
    for (uint32_t j = 0; j < n_buckets; j++) {
        ...
        for (; iter; ) {
            struct wc_word *w =
                valid ? container_of(iter, struct wc_word, node_main)
                      : container_of(iter, struct wc_word, node);
            iter = ht_get_next(&wcc->htable, j, iter) ;
            free(w->full_word);
            free(w);
        }
    }
...
static __thread char * start  = NULL ;
...
{
    if (count) {
        if (wc_add_word(tid, start, count)) return -1;
        count = 0;
    }
    return 0;
}
